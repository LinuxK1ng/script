...
I2P:${alignr}${if_match ${execi 20 sh -c 'i2prouter status 2>/dev/null | grep -qi running && echo 1 || echo 0'} >= 1}${color4}ENABLED${color}${else}${color2}DISABLED${color}${endif}
DNSCrypt-Proxy:${alignr}${if_match ${execi 30 sh -c 'systemctl is-active dnscrypt-proxy.service 2>/dev/null | grep -qi active && echo 1 || echo 0'} >= 1}${color4}ENABLED${color}${else}${color2}DISABLED${color}${endif}
